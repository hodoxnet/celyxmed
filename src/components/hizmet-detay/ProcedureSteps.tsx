  ...
  return (
    <section className="py-16 md:py-24 bg-gray-50 dark:bg-gray-900">
      <div className="container mx-auto px-4">
        ...
        >
          {steps.map((step) => (
            <div key={step.id} className="flex-shrink-0 w-full sm:w-[48%] md:w-[32%] lg:w-[32%] p-8 pt-10 bg-white dark:bg-gray-800 rounded-xl shadow-sm border-t-4 border-[#d4b978] dark:border-[#a88d5f] flex flex-col min-h-[280px]">
              <h3 className="text-2xl font-semibold mb-4 text-gray-900 dark:text-white">{step.title}</h3>
              <p className="text-gray-600 dark:text-gray-400 text-base leading-relaxed mb-6 flex-grow">{step.description}</p>
              {step.linkText && (
                 <Link href={step.linkHref || '/iletisim'} className="mt-auto self-start inline-flex items-center text-base font-medium text-gray-900 dark:text-white hover:underline group">
                   {step.linkText}
